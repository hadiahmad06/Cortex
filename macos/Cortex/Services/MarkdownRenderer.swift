...
struct MarkdownRenderer {
  static private func safeMarkdown(_ text: String) -> [MarkdownSegment] {
    var markdown = text
  ...
  static private func getMarkdown(_ markdown: String, text: String) -> [MarkdownSegment] {
  ...
  static func render(_ text: String, incoming: Bool = false) -> ([AnyView], Bool) {
    var isInlineOnly: Bool = true
    var segments: [AnyView] = []
    var buffer = AttributedString()
    let markdown = incoming ? safeMarkdown(text) : getMarkdown(text, text: text)
    for segment in markdown {
      switch segment {
